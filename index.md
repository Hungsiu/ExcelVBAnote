...
※小提醒：我用的版本是2016，所以按鈕或是功能的位置也許會有一些差異，在操作時需要留意
...
首先是頭尾的Sub Button_Click()與End Sub，這兩行定義出一個函數的區域，這個區域內可以寫入你需要的程式碼。Sub這個關鍵字敘述函數的起點，而Button_Click是這個函數的名字，並依照語法規定在名稱後加上括號「()」，最後以End Sub作為結束這個區域的敘述

接下來是兩行Cells，這是使用名叫Cells函數的敘述，這個函數能夠指出儲存格的位置，而括號內需要給它2個參數，分別是列與欄。因此在第一次呼叫Cells時，我希望它能幫我指出A1這個位置的儲存格，因此在第一個參數－列填入1，並用逗號「,」將複數個參數分開，在第二個參數－欄中填入"A"，因此Cells(1,"A")就會指出A1這個儲存格。而我指出A1這個儲存格後要做什麼呢？我把"用Cells寫的Hello"這段文字透過等號「=」輸入進A1這個儲存格。第二次呼叫Cells時在欄的參數中填入2，這時候Cells指出B1這個儲存格，所以我們可以得知其實A、B、C、D....這些英文字母是對應1、2、3、4的，只是這樣會變德很難區分列與欄（但在之後完全用程式時還是需要這樣）

另一種指出儲存格的方式是透過Range這個函數，但與Cells不同，Range只能帶入一個參數，因此使用Range時就需要直接給它希望指出來的儲存格位置，像是第一次呼叫Range時，希望它直接指出A2這個儲存格，因此在參數內填入"A2"。
